...
class DoubleDeepQLearningAgent(Agent):
    def __init__(self, input_size, action_size, num_layers=3, num_hidden_per_layer=256, epsilon=0.01, lr=0.01,
        ...
        self.A = self.create_net(input_size, action_size, num_layers, num_hidden_per_layer)
        self.QaTarget = self.create_net(input_size, action_size, num_layers, num_hidden_per_layer)
        self.QaTarget.set_weights(self.A.get_weights())

        self.V = self.create_net(input_size, action_size, num_layers, num_hidden_per_layer)
        self.QbTarget = self.create_net(input_size, action_size, num_layers, num_hidden_per_layer)
        self.QbTarget.set_weights(self.V.get_weights())
